...
| discipline | string | Semantic for groups which refine the user experience. `Overview` will indicate that this layer may not be filtered.<div>Possible values are:<ul><li>`Overview`</li><li>`FullModel`</li><li>`Mechanical`</li><li>`Architectural`</li><li>`Piping`</li><li>`Electrical`</li><li>`Structural`</li></ul></div> |
