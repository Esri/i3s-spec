...
1. The offset of each file is known (i.e. byte offset from the beginning of the slpk file to the first byte of its ZIP local file header (see ZIP specs for reference) 
2. Convert all file paths to their *canonical* form: lower case, forward-slash path separator (`/`), no heading forward-slash (e.g. `/my/PATH.json` becomes `my/path.json`).
...
```
5. Write this sorted array as the last file of the SLPK archive (**last entry** in the ZIP central directory). Each array element is 24-bytes long (16 bytes for the MD5-digest and 8 bytes for the offset) in little-endian order, no padding, no header. File **must be named** `@specialIndexFileHASH128@`
